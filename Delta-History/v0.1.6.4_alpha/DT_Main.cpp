...
int main()
{
	...
	int selector1;
	printf("------------------------------------------------\n");
	printf("Delta.-Version:v0.1.6.4_alpha.\n");
	...
	scanf_s("%d", &selector1);  /*=运算符不正确: 在 Boolean 上下文中执行了常量赋值。应考虑改用“==”*/
	if (selector1 == 1) /*The main selector.Now executes if its value is 1*/
	{
		int fselector;
		printf("Please select: 1. C=>F | 2. F=>C\n");
		scanf_s("%d", &fselector);
		if (fselector == 1)  /*F<=>C Selector executes the sentences below*/
		{
